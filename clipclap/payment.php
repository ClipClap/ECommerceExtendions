...
include_once(_PS_MODULE_DIR_.'/clipclap/clipclap.php');

class ClipClapController extends FrontController
{
	public $ssl = true;
	...
	public function process()
	{
		...
		self::$smarty->assign(array(
				'formLink' => Configuration::get('CLIPCLAP_DEMO') != 'yes' ? 'https://gatewaylap.pagosonline.net/ppp-web-gateway/' :
				'https://stg.gatewaylap.pagosonline.net/ppp-web-gateway/',
				'ClipClapRedirection' => $params
			));
	}
	...
	public function displayContent()
	{
		...
		self::$smarty->display(_PS_MODULE_DIR_.'clipclap/views/templates/front/redirect.tpl');
	}
...
public function initParams()
{
	...
	$ref = 'clipclap_'.Configuration::get('PS_SHOP_NAME').'_'.(int)self::$cart->id;

	$token = md5(Tools::safeOutput(Configuration::get('CLIPCLAP_API_KEY')).'~'.Tools::safeOutput(Configuration::get('CLIPCLAP_MERCHANT_ID')).'~'.$ref.'~'
	.(float)self::$cart->getOrderTotal().'~'.Tools::safeOutput($currency->iso_code));

	$params = array(
		array('value' => (Configuration::get('CLIPCLAP_DEMO') == 'yes' ? 1 : 0), 'name' => 'test'),
		array('value' => Tools::safeOutput(Configuration::get('CLIPCLAP_MERCHANT_ID')), 'name' => 'merchantId'),
		...
		'responseUrl'),
		array('value' => 'http://'.htmlspecialchars($_SERVER['HTTP_HOST'], ENT_COMPAT, 'UTF-8').__PS_BASE_URI__.'modules/clipclap/validation.php', 'name' => 'confirmationUrl'),
	);

	if (Configuration::get('CLIPCLAP_ACCOUNT_ID') != 0)
		$params[] = array('value' => (int)Configuration::get('CLIPCLAP_ACCOUNT_ID'), 'name' => 'accountId');
	...
	public function createPendingOrder()
	{
		...
		$clipclap->validateOrder((int)self::$cart->id, (int)Configuration::get('CLIPCLAP_WAITING_PAYMENT'), (float)self::$cart->getOrderTotal(),
		$clipclap->displayName, null, array(), null, false,	self::$cart->secure_key);
	}
}

$ClipClapController = new ClipClapController();

if (Tools::getIsset(Tools::getValue('create-pending-order')))
	$ClipClapController->createPendingOrder();
else
	$ClipClapController->run();
