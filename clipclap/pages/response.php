...
include(dirname(__FILE__).'/../clipclap.php');
...
$api_key = Configuration::get('CLIPCLAP_API_KEY');
...
if ($transaction_state == 6 && $pol_response_code == 5)
	$estado_tx = $clipclap->l('Failed Transaction');
else if ($transaction_state == 6 && $pol_response_code == 4)
	$estado_tx = $clipclap->l('Rejected Transaction');
else if ($transaction_state == 12 && $pol_response_code == 9994)
	$estado_tx = $clipclap->l('Pending Transaction, Please check if the debit was made in the Bank');
else if ($transaction_state == 4 && $pol_response_code == 1)
{
	$estado_tx = $clipclap->l('Transaction Approved');
	$messageApproved = $clipclap->l('¡Thank you for your purchase!');
}
else
{
...
if (Tools::strtoupper($signature) == Tools::strtoupper($signature_md5))
{
	if (!($cart->orderExists()))
	{
		...
		$clipclap->validateOrder((int)$cart->id, Configuration::get('CLIPCLAP_OS_PENDING'), (float)$cart->getordertotal(true), 'ClipClap', null, array(), (int)$cart->id_currency, false, $customer->secure_key);
	}
